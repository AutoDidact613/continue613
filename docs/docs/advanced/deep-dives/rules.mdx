...
When in Agent mode, you can prompt the agent to create a rule for you using the `create_rule_block` tool if enabled.
...
Explore available rules [here](https://hub.continue613.dev), or [create your own](https://hub.continue613.dev/new?type=block&blockType=rules) in the Hub.
