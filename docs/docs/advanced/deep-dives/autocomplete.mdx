...
<Tabs groupId="hub-config-example">
  ...
  [Mistral Codestral model block](https://hub.continue613.dev/mistral/codestral)
  </TabItem>
...
<Tabs groupId="hub-config-example">
  <TabItem value="hub" label="Hub">
  [Ollama Qwen 2.5 Coder 1.5B model block](https://hub.continue613.dev/ollama/qwen2.5-coder-1.5b)
  </TabItem>
  <TabItem value="yaml" label="YAML">
  ```yaml title="config.yaml"
  models:
...
Explore autocomplete model configurations on [the hub](https://hub.continue613.dev/explore/models?roles=autocomplete)
