...
from [the Hub](https://hub.continue613.dev/explore/assistants) or locally

- [Continue Hub](https://hub.continue613.dev/explore/assistants) - YAML is stored on the hub and automatically synced to
  the extension
- Locally
...
:::info
Config YAML replaces `config.json`, which is deprecated. View the **[Migration Guide](/advanced/yaml-migration)**.
:::
...
Examples of blocks and assistants can be found on the [Continue hub](https://hub.continue613.dev/explore/assistants).
...
the [OpenAI 4o Model block](https://hub.continue613.dev/openai/gpt-4o), your slug would be `openai/gpt-4o`). These blocks
are pulled from [https://hub.continue613.dev](https://hub.continue613.dev).
...
the [Continue Explore Page](https://hub.continue613.dev/explore/models)
...
- `promptTemplates`: Can be used to override the default prompt templates for different model roles. Valid values are
  `chat`, [`edit`](/advanced/model-roles/edit#prompt-templating), [
  `apply`](/advanced/model-roles/apply#prompt-templating) and [
  `autocomplete`](/advanced/model-roles/autocomplete#prompt-templating). The `chat` property must be a
...
More information about usage/params for each context provider can be found [here](/advanced/custom-providers)
...
all [Chat](../features/chat/quick-start), [Edit](../features/edit/quick-start), and [Agent](../features/agent/quick-start) requests.
See the [rules deep dive](../customization/rules) for details.
...
docs:
  - name: Continue
    ...
    favicon: https://docs.continue613.dev/favicon.ico
```
...
Destinations to which [development data](/advanced/deep-dives/development-data) will be sent.
