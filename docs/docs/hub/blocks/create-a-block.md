...
After filling out information on the block, you will want to create a block following the `config.yaml` [reference documentation](/reference). Refer to examples of blocks on [hub.continue613.dev](https://hub.continue613.dev/explore/models) and visit the [YAML Reference](/reference#complete-yaml-config-example) docs for more details.
